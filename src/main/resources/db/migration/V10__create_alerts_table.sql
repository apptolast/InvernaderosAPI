...
ALTER TABLE metadata.alerts
    DROP CONSTRAINT IF EXISTS fk_alerts_tenant;
...
ALTER TABLE metadata.alerts
    DROP CONSTRAINT IF EXISTS fk_alerts_resolved_by_user;
...
ALTER TABLE metadata.alerts
    ADD CONSTRAINT chk_alert_severity
        CHECK (severity IN ('INFO', 'WARNING', 'ERROR', 'CRITICAL', 'LOW', 'MEDIUM', 'HIGH'));

-- Paso 8: Agregar CHECK constraint para consistencia de resolved
ALTER TABLE metadata.alerts
    DROP CONSTRAINT IF EXISTS chk_resolved_consistency;

ALTER TABLE metadata.alerts
    ADD CONSTRAINT chk_resolved_consistency
        CHECK (
...
CREATE INDEX IF NOT EXISTS idx_alerts_tenant_unresolved
    ON metadata.alerts(tenant_id, is_resolved, created_at DESC)
