 ...
 *
 * Utiliza Redis Sorted Set para almacenar los mensajes ordenados por timestamp, permitiendo
...
class GreenhouseCacheService(private val redisTemplate: RedisTemplate<String, String>) {
    ...
    companion object {
        ...
        /**
         * Genera la cache key específica para un tenant Formato: "greenhouse:messages:{tenantId}"
         *
    ...
    fun cacheMessage(message: RealDataDto) {
        try {
            ...
            logger.debug(
                    "Mensaje cacheado en Redis para tenant=$tenantId: timestamp=${message.timestamp}"
            )
        } catch (e: Exception) {
    ...
    fun getMessagesByTimeRange(
        ...
        return try {
            ...
            logger.error(
                    "Error al obtener mensajes por rango de tiempo desde Redis para tenant=$tenantId",
                    e
            )
            emptyList()
        }
    ...
    /** Obtiene estadísticas de la caché para un tenant */
