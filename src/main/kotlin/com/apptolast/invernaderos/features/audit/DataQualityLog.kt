...
/**
 * Entity para registrar problemas de calidad de datos detectados en el sistema. Ayuda a identificar
 * datos faltantes, duplicados, fuera de rango, inconsistentes, etc.
 *
 * @property id ID único del registro (BIGSERIAL)
 * @property dataSource Fuente de datos donde se detectó el problema
 * @property qualityIssueType Tipo de problema: MISSING_DATA, OUT_OF_RANGE, DUPLICATE, INCONSISTENT,
 * etc.
...
data class DataQualityLog(
        ...
        /**
         * Fuente de datos donde se detectó el problema. Ejemplos: "MQTT_BROKER", "SENSOR_READINGS",
         * "TIMESCALEDB", "API"
         */
        @Column(name = "data_source", nullable = false, length = 50) val dataSource: String,

        /**
         * Tipo de problema de calidad de datos. Ejemplos: MISSING_DATA, OUT_OF_RANGE, DUPLICATE,
         * INCONSISTENT, STALE_DATA, ANOMALY
         */
        ...
        @Column(length = 20) val severity: String? = null,
        ...
        @Column(name = "resolved_by") val resolvedBy: UUID? = null,
        @Column(name = "detected_at", nullable = false) val detectedAt: Instant = Instant.now(),

        /** Inicio del rango temporal afectado por el problema. */
        @Column(name = "time_range_start") val timeRangeStart: Instant? = null,

        /** Fin del rango temporal afectado por el problema. */
        @Column(name = "time_range_end") val timeRangeEnd: Instant? = null,

        /** Cantidad de registros afectados por el problema. */
        @Column(name = "affected_records_count") val affectedRecordsCount: Int? = null,
        @Column(nullable = false, columnDefinition = "TEXT") val description: String,

        /**
         * Datos de ejemplo del problema en formato JSONB. Ejemplo: {"sensor_id": "TEMP01",
         * "expected_range": [0, 50], "actual_value": 999}
         */
         ...
         */
        @Column(nullable = false, length = 20) val status: String = Status.OPEN,
