...
class SimulationController(
        ...
        @Operation(
                ...
                description =
                        "Genera un conjunto de datos simulados y los procesa inmediatamente (Redis + DB + WebSocket)"
        )
        @ApiResponses(
                value =
                        [
        ...
        fun generateSimulatedData(
                ...
                return try {
                        logger.info(
                                "Generando datos simulados bajo demanda para greenhouse: {}",
                                greenhouseId
                        )
                        ...
                        val problemDetail =
                                ProblemDetail.forStatusAndDetail(
                                        ...
                                        "Error generating simulated data: ${e.message ?: "Unknown error"}"
                                )
                        problemDetail.title = "Simulation Error"
                        problemDetail.setProperty("timestamp", java.time.Instant.now())
                        problemDetail.setProperty("path", "/api/simulation/generate")
        ...
        fun getSimulationStatus(): ResponseEntity<SimulationStatus> {
                ...
                val status =
                        SimulationStatus(
                                ...
                                message =
                                        "Simulación activa - Generando datos cada ${"%.1f".format(properties.intervalMs / 1000.0)} segundos"
                        )
        ...
        @Operation(
                ...
                description =
                        "Genera datos simulados SIN procesarlos (no se guardan en BD ni se envían por WebSocket)"
        )
        @ApiResponses(
                value =
                        [
        ...
        fun previewSimulatedData(
                ...
                logger.debug(
                        "Generando preview de datos simulados para greenhouse: {}",
                        greenhouseId
                )
        ...
        fun getInfo(): ResponseEntity<SimulationInfo> {
                ...
                val info =
                        SimulationInfo(
                                version = "1.0",
                                description =
                                        "API para simular datos de invernadero cuando sensores físicos no están disponibles",
                                endpoints =
                                        listOf(
                                                EndpointInfo(
                                                        method = "POST",
                                                        ...
                                                        description =
                                                                "Genera y procesa datos simulados inmediatamente",
                                                        parameters =
                                                                listOf("greenhouseId (opcional)")
                                                ),
                                                EndpointInfo(
                                                        method = "GET",
                                                        path = "/api/simulation/status",
                                                        description =
                                                                "Ver estado y configuración de la simulación",
                                                ...
                                                EndpointInfo(
                                                        ...
                                                        description =
                                                                "Generar datos sin procesarlos (preview)",
                                                        parameters =
                                                                listOf("greenhouseId (opcional)")
                                                ),
                                                EndpointInfo(
                                ...
                                dataRanges =
                                        DataRanges(
                                                ...
                                                humidity = "30-90%",
                                                sectors =
                                                        "0-100% (70% probabilidad 30-70%, 30% extremos)",
                                                extractors =
                                                        "0.0 o 1.0 (30% probabilidad encendido)",
                                                reserva = "0-100"
                                        ),
                                notes =
                                        listOf(
