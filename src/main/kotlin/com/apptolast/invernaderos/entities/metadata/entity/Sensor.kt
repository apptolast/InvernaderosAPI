...
/**
 * Entity que representa un Sensor en un invernadero.
 * Los sensores recopilan datos (temperatura, humedad, etc.) y envían vía MQTT.
 *
 ...
 * @property mqttFieldName Nombre del campo en payload JSON MQTT (ej: empresaID_sensorID, TEMPERATURA_INVERNADERO_01)
...
@Table(
    ...
    indexes = [
        ...
        Index(name = "idx_sensors_greenhouse_active", columnList = "greenhouse_id, is_active"),
        Index(name = "idx_sensors_type_active", columnList = "sensor_type, is_active")
    ],
    uniqueConstraints = [
        UniqueConstraint(name = "uq_sensor_code_per_greenhouse", columnNames = ["greenhouse_id", "sensor_code"]),
        UniqueConstraint(name = "uq_mqtt_field_per_greenhouse", columnNames = ["greenhouse_id", "mqtt_field_name"])
    ]
)
data class Sensor(
    @Id
    ...
    /**
     * Código corto único del sensor dentro del greenhouse.
     * Ejemplo: "TEMP01", "HUM02", "PRES01"
     ...
     * Valores posibles: NUMERIC, STRING, JSON, BOOLEAN
     * Default: NUMERIC
     */
    @Column(name = "data_format", length = 20)
    val dataFormat: String? = "NUMERIC",
     ...
     * Un sensor pertenece a un greenhouse.
     */
    ...
    /**
     * Relación ManyToOne con Tenant (denormalizado).
     * Un sensor pertenece a un tenant.
     */
    @ManyToOne(fetch = FetchType.LAZY)
